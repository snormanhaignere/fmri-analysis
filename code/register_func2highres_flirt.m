...
if betweight
    ...
    exfunc_betmask_swaplr = [preprocdir 'bet_mask_swaplr.nii.gz']; % brain-masked functional image used for motion correction
...
if optInputs(varargin, 'swaplr')
    unix_fsl(fsl_version, ['fslswapdim ' exfunc ' -x y z ' exfunc_swaplr]);
    exfunc = exfunc_swaplr;
    if betweight
        unix_fsl(fsl_version, ['fslswapdim ' exfunc_betmask ' -x y z ' exfunc_betmask_swaplr]);
        exfunc_betmask = exfunc_betmask_swaplr;
    end
end
