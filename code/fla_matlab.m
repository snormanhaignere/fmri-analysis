function S = fla_matlab(exp,us,runtype,r,hrf_name,volume_or_surface,input_fname,output_directory_name,varargin)
...
rmpath('/mindhive/nklab/u/svnh/export_fig_v2');
addpath('/mindhive/nklab/u/svnh/export_fig_v3');
...
if  ~exist(design_file,'file') || ~exist(beta_file,'file') || ~exist(psc_file,'file') || ~exist(rvar_file,'file') || ~exist(beta_white_file,'file') || ~exist(psc_white_file,'file') || ~exist(rvar_white_file,'file') || optInputs(varargin, 'overwrite')
    ...
    if optInputs(varargin, 'permute_condition_order')
        ...
        for q = 1:n_perms
            if n_folds > 1
                for i = 1:n_folds
                    xi = randperm(n_conds_per_fold);
                    conds_reshaped(:,i) = conds_reshaped(xi,i);
                end
                clear xi;
                conds_permuted = conds_reshaped(:);
            else
    ...
    if optInputs(varargin, 'block-nuissance')
        
        blocks = varargin{optInputs(varargin, 'block-nuissance')+1};
        
        assert(sum(blocks) == nTR);
        
        n_blocks = length(blocks);
        X_block = zeros(nTR, n_blocks);
        for i = 1:n_blocks
            X_block((1:blocks(i)) + sum(blocks(1:i-1)),i) = 1;
        end
        
        X_nuissance = [X_nuissance, X_block];
    ...
    if n_whitematter_PCs > 0
        ...
        if ~exist(whitematter_mat_file,'file') || optInputs(varargin, 'overwrite')
            ...
            % remove existing nuissance regressors
            if ~isempty(X_nuissance)
                wm_matrix = wm_matrix - X_nuissance * (pinv(X_nuissance) * wm_matrix);
            end
...
% condition and timing information
evname = read_conditions(exp,us,runtype,'run',r,varargin{:});
nevs = length(evname);
if optInputs(varargin, 'contrasts')
    contrastnames = varargin{optInputs(varargin,'contrasts')+1};
else
    contrastnames = read_contrasts(exp,us,runtype,[],'run',r,varargin{:});
end
...
for c = 1:length(contrastnames);
    ...
    if length(evgroup)==1
        ...
        evinds = strmatchMANY( evgroup2evname(exp,us,evgroup{1},[],'run',r,varargin{:}),  evname );
        if isempty(evinds); error(['Failed to match contrasts to evs: ' contrastnames{c}]); end
        contrasts(c,evinds) = 1;
        
    else
        
        evinds1 = strmatchMANY( evgroup2evname(exp,us,evgroup{1},[],'run',r,varargin{:}),  evname );
        evinds2 = strmatchMANY( evgroup2evname(exp,us,evgroup{2},[],'run',r,varargin{:}),  evname );
...
[~,~,cc] = read_contrasts(exp,us,runtype,[],'run',r);
if ~isempty(cc)
...
for i = 1:size(contrasts,1)
    ...
    if optInputs(varargin, 'permute_condition_order')
        switch volume_or_surface
            case {'volume', 'downsampled_surface'}
                ...
                p_file_permuted_gaussfit = [contrast_directory 'permuted_pstat_gaussfit_' contrastnames{i} '_gridded.mat'];
                S.p_file_permuted_gaussfit{i,1} = contrastnames{i};
                S.p_file_permuted_gaussfit{i,2} = p_file_permuted_gaussfit;
            case 'surface'
    ...
    if optInputs(varargin, 'permute_condition_order') && (~exist(perm_cond_cope_file,'file') || optInputs(varargin,'overwrite'))
        ...
        if strcmp(volume_or_surface, 'downsampled_surface')
            ...
            save(p_file_permuted_gaussfit,'G');
            Gpstat_gaussfit = G;
