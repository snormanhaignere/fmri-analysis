function [rh_mask, lh_mask, bootstrapped_cluster_sizes, tla_directory] = tla_permutation2(P,volume_or_surface,tla_directory_name,varargin)

% Calculates voxelwise thresholds for cluster-correction using p-values instead of z-values.
% 
...
if ~all_files_exist || optInputs(varargin, 'overwrite')
    ...
    save(mat_file_with_all_useful_statistics, 'zstat', 'pstat', 'pstat_gaussfit', 'zstat_gaussfit', 'nullmean', 'nullstd', 'permuted_lower_level_z_with_noNaNs', 'voxels_with_no_NaNs', 'nullmean_voxels_with_no_NaNs', 'nullstd_voxels_with_no_NaNs', 'smps')
...
% bootstrapped cluster sizes
matfile_variables = whos('-file',mat_file_with_all_useful_statistics);
bootstrap_variable_present = false;
for i = 1:length(matfile_variables)
    if strcmp(matfile_variables(1).name, 'bootstrapped_cluster_sizes')
        bootstrap_variable_present = true;
    end
end

if ~bootstrap_variable_present || optInputs(varargin, 'overwrite')

    %     fid = fopen('~/freesurfer/fsaverage/surf/rh.area.asc','r');
    %     vertarea_rh = fscanf(fid,'%f'); fclose(fid);
    %
    %     fid = fopen('~/freesurfer/fsaverage/surf/lh.area.asc','r');
    %     vertarea_lh = fscanf(fid,'%f'); fclose(fid);
    %
    subject_smps = randi(n_subjects, [n_subjects, n_smps]);
    fprintf('Boot-strapped cluster sizes\n');
    tic;
    bootstrapped_cluster_sizes = zeros(n_smps,2);
    for i = 1:n_smps
        ...
        for j = 1:comp_rh.NumObjects
            ...
            if clust_size > cluster_size_thresh
                bootstrapped_cluster_sizes(i,1) = bootstrapped_cluster_sizes(i,1) + clust_size;
            end
        end
        ...
        for j = 1:comp_lh.NumObjects
            ...
            if clust_size > cluster_size_thresh
                bootstrapped_cluster_sizes(i,2) = bootstrapped_cluster_sizes(i,2) + clust_size;
            end
        end
    ...
    save(mat_file_with_all_useful_statistics, 'bootstrapped_cluster_sizes', '-append');
    
else
    
    load(mat_file_with_all_useful_statistics, 'bootstrapped_cluster_sizes');
