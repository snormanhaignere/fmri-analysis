...
sladir = [params('rootdir') 'freesurfer/' subjid '/sla/'];
if ~exist(sladir,'dir');
    mkdir(sladir);
end

emptysurf = MRIread([params('rootdir') 'freesurfer/fsaverage/surf/' hemi '.area.mgh']);
emptysurf.fspec = '';
...
for i = 1:length(freqs)
    ...
    glmdir = [params('rootdir') 'freesurfer/fsaverage/sla/' subjid '/' hemi '_' idstring '_r' sprintf('%d',runnum)];
...
bestfreq_file = [params('rootdir') 'freesurfer/fsaverage/sla/' subjid '/' hemi '.bestfreq_' idstring '.mgh'];
bestfreq = emptysurf;
...
if optInputs(varargin, 'cluscorr')
    anova_surf = [params('rootdir') 'freesurfer/fsaverage/sla/' subjid '/' hemi '.' 'anova_' sprintf('%s',condlevels{:}) '_cluscorr_fwhm' num2str(fwhm) '_projfrac' num2str(projfrac(1)) '-' num2str(projfrac(2)) '_trilinear_' runtype '_r' sprintf('%d',runnum) '_' model  '_' num2str(fwhm_vol*100,'%.0f') 'mm.mgz'];
else
    anova_surf = [params('rootdir') 'freesurfer/fsaverage/sla/' subjid '/' hemi '.' 'anova_' sprintf('%s',condlevels{:}) '_fwhm' num2str(fwhm) '_projfrac' num2str(projfrac(1)) '-' num2str(projfrac(2)) '_trilinear_' runtype '_r' sprintf('%d',runnum) '_' model  '_' num2str(fwhm_vol*100,'%.0f') 'mm.mgz'];
end
if ~exist(anova_surf,'file') || optInputs(varargin,'overwrite')
    ...
    refvol = [params('rootdir') 'freesurfer/' subjid '/mri/orig.mgz'];
...
if optInputs(varargin,'mask_label')
    ...
    lb = read_label([params('rootdir') 'freesurfer/fsaverage/label/' hemi '.' label '.label']);
...
%     labelfile = [params('rootdir') 'freesurfer/fsaverage/sla/pitch_resthr_v4_us' num2str(us) '/' hemi '_harm-f0333_vs_noise-f0333_fwhm0_projfrac0-1_trilinear_localizer_block_3mm_r' sprintf('%d',runnum) '/osgm/sig_stp_v3.bin.smooth3.bin.label'];
...
% end
    ...
    %     inflated_file = [params('rootdir') 'freesurfer/fsaverage/surf/' hemi '.inflated'];
    %     if strcmp(hemi,'rh')
    %         cam = [200 20 20];
...
%
%     area_file = [params('rootdir') 'freesurfer/fsaverage/surf/' hemi '.area2.mgh'];
...
%
% area_file = [params('rootdir') 'freesurfer/fsaverage/surf/' hemi '.area.mgh'];
% area_copy_file = [params('rootdir') 'freesurfer/fsaverage/surf/' hemi '.area2.mgh'];
