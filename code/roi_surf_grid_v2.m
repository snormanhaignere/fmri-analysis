...
fla_output_fname = fla_input_fname;

% additional flags to the file name to specify this analysis
if ~isempty(test_info.fla_fname_flags)
    fla_output_fname = [fla_output_fname '_' test_info.fla_fname_flags];
else
    assert(isempty(test_info.fla_optargs));
end
...
S = fla_matlab(...
    ...
    varargin{:}, test_info.fla_optargs{:});
...
% additional flags to the file name to specify this analysis
if ~isempty(localizer_info.fla_fname_flags)
    fla_output_fname = [fla_output_fname '_' localizer_info.fla_fname_flags];
else
    assert(isempty(localizer_info.fla_optargs));
end
...
S = fla_matlab(...
    ...
    varargin{:}, localizer_info.fla_optargs{:});
...
% additional flags to the file name to specify this analysis
if ~isempty(localizer_info.fla_fname_flags)
    fla_fname = [fla_fname '_' localizer_info.fla_fname_flags];
else
    assert(isempty(localizer_info.fla_optargs));
end
...
% if strcmp(localizer_info.runtype, 'pitchloc7_70-75-80dB_combined_split')
%     grid_file = ['smooth' num2str(fwhm*100, '%.0f') '-volume' ...
%         'mm_grid_' grid_roi '_' num2str(grid_spacing_mm) 'mm_10whitematterPCs_blocknuissance'];
% elseif strcmp(localizer_info.runtype, 'mvocs_pitch_v5_combined_split')
%     grid_file = ['smooth' num2str(100*fwhm, '%.0f') 'mm-surface' ...
%         '_grid_' roi '_' num2str(grid_spacing_mm) 'mm_10whitematterPCs'];
% else
...
switch localizer_info.stat_type
    ...
    case 'perm'
        ...
        for q = 1:length(localizer_runs_to_use);
            ...
            P(q).lower_level_directory_name = fla_fname;
        end
        ...
        fprintf('Need to check this accurate\n'); drawnow;
        keyboard;
...
for j = 1:n_localizers
    ...
    if ~isfield(localizer_info(j), 'fla_optargs')
        localizer_info(j).fla_optargs = {};
    end
    
    if ~isfield(localizer_info(j), 'fla_fname_flags')
        localizer_info(j).fla_fname_flags = '';
    end
end
...
% conditions to measure responses to
if ~isfield(test_info, 'fla_optargs')
    test_info.fla_optargs = {};
end

if ~isfield(test_info, 'fla_fname_flags')
    test_info.fla_fname_flags = '';
end